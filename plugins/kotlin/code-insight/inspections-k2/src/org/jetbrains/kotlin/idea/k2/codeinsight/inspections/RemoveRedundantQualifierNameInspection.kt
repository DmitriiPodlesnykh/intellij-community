...
class RemoveRedundantQualifierNameInspection :
     ...
     * redundant qualifier.
     */
    private var unwrapFakeOverrides: Boolean = false
    ...
    context(KtAnalysisSession) private fun prepareContextForQualifier(expression: KtDotQualifiedExpression): Context? {
        ...
        when {
            isEnumCompanionObject(receiverDeclaration) -> if (receiver is KtDotQualifiedExpression) {
                ...
                /**
                 * Shorten if it is already imported.
                 * TODO: Add the following example as a test:
                 * import EnumClass.Companion.foo
                 * enum class EnumClass(val i: Int) {
                 *   A(1)
                 *   ;
                 *   companion object { fun foo() = "" }
                 * }
                 * fun test() = EnumClass.Companion.foo()   // can be foo()
                 */
            }
        ...
        /*
        val qualifierExpression = expressionForAnalyze as? KtDotQualifiedExpression ?: return null
        val symbol = qualifierExpression.selectorExpression?.let { selector ->
            when (selector) {
                is KtCallExpression -> selector.calleeExpression?.mainReference
                is KtReferenceExpression -> selector.mainReference
                else -> null
            }
        }?.resolveToSymbol() ?: return null
        ...
        while (true) {
            ...
            if (!shortenCommand.isEmpty) {
                ...
                shortenings.addAll(getAllShortenings(expressionForAnalyze))
    ...
    context(KtAnalysisSession) private fun tryShorteningExpression(expression: KtDotQualifiedExpression, symbol: KtSymbol) =
        collectPossibleReferenceShortenings(
            expression.containingKtFile, TextRange(expression.startOffset, expression.endOffset),
            {
    ...
    context(KtAnalysisSession) private fun prepareContextForUserType(type: KtUserType): Context? {
        val shortenCommand = collectPossibleReferenceShortenings(
            type.containingKtFile, TextRange(type.startOffset, type.endOffset),
            { ShortenOption.SHORTEN_IF_ALREADY_IMPORTED },
            { ShortenOption.SHORTEN_IF_ALREADY_IMPORTED },
        )
        if (!shortenCommand.isEmpty) {
            val shortenings = mutableListOf(shortenCommand)
            shortenings.addAll(getAllShortenings(type))
            return Context(shortenings)
        }
        return null
    }

    context(KtAnalysisSession) private fun getAllShortenings(element: KtElement): List<ShortenCommand> {
        ...
        val range = when (element) {
            ...
            is KtDotQualifiedExpression -> {
                val selectorReference = element.selectorExpression?.mainReference?.resolveToSymbol()
                val endOffset = if (isEnumClass(selectorReference) || isEnumCompanionObject(selectorReference)) {
                    element.endOffset
                } else {
                    element.getLastParentOfTypeInRowWithSelf<KtDotQualifiedExpression>()?.getQualifiedElementSelector()?.endOffset
                        ?: return emptyList()
                }
        ...
        Regex.fromLiteral(substring).findAll(file.text, file.importList?.endOffset ?: 0).toList().asReversed().forEach {
            if (range.first == it.range.first) return@forEach
            val shortenCommand = collectPossibleReferenceShortenings(file,
                                                                     TextRange(it.range.first, it.range.last + 1),
                                                                     { ShortenOption.SHORTEN_IF_ALREADY_IMPORTED },
                                                                     { ShortenOption.SHORTEN_IF_ALREADY_IMPORTED })
            if (!shortenCommand.isEmpty) shortenings.add(shortenCommand)
        }
        return shortenings
    }
    ...
    fun KtDotQualifiedExpression.isApplicableByPsi(): Boolean {
        /**
         * The following three lines avoids handling [KtDotQualifiedExpression] whose a child of [KtDotQualifiedExpression],
        ...
        /**
         * This will above the situation where removing a qualifier results in a name conflict.
...
context(KtAnalysisSession) private fun KtExpression.getQualifiedElementSelectorDeclaration() =
