...
internal class MovePropertyToConstructorIntention :
    ...
    override fun prepareContext(element: KtProperty): Context? {
        ...
        if (constructorParameter != null) {
            ...
            val typeText = element.typeReference?.text ?: run {
                val type = element.getVariableSymbol().returnType
                type.render(KtTypeRendererForSource.WITH_QUALIFIED_NAMES, Variance.INVARIANT)
            }
    ...
    context(KtAnalysisSession)
    private fun KtType.isKCallable(): Boolean {
        val kCallableStarKtType = buildClassType(StandardClassIds.KCallable) {
            argument(KtStarTypeProjection(token))
        }
        return this.isSubTypeOf(kCallableStarKtType)
    }
